...
- The Firezone server `services.firezone.server` package and module have been updated and upstream now uses a unified portal architecture instead of splitting the server into web, api and domain. As a consequence, the module structure has changed and the previous multi-component setup had to be consolidated into a single service. Component specific settings were removed. Options were lifted into the `services.firezone.server` scope. Environment variables can be transferred into the global option, and domain specific settings are now exposed under `services.firezone.server.portal`. Furthermore, service accounts no longer belong to the autogenerated everyone group in Firezone. If you have used the everyone group to add permissions to a service account, please make sure to create a separate group for these.
